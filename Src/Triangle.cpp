...
double Triangle::raytrace(Ray& ray) const {
    float u, v; //not used here
    return intersection(ray.rayPos + ray.rayDir * offset, ray.rayDir, u, v);
}
...
Vec3f Triangle::normalAtPoint(Vec3f fragPos, const Ray& ray) const {
